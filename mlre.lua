...
--local scale = controlspec.new(-1.0,1.0,"lin", 5/12, 0,"", 1/36) -- fourths
--local scale = controlspec.new(-1.0,1.0,"lin", 6/12, 0,"", 1/36) -- diminished fifths
...
--norns.enc.sens(1,6) -- set enc sensitivity for page turn
...
retrig = function() -- add retrig function for playing tracks (see gridnav)
  for i=1,TRACKS do
...
-- for hnds (TODO: need to tweek the min max values for the according target params)
...
init = function()
  ...
  params:add_control("sc_input_level", "input dry -> sc level", controlspec.new(0, 1, "lin", 0, 0))
  params:set_action("sc_input_level", function(x) audio.level_adc_cut(x) end)

  -- engine to softcut level
  params:add_control("sc_engine_level", "delay mix -> sc level", controlspec.new(0, 1, "lin", 0, 1))
...
-- params for tracks 1-6
  ...
  --softcut settings
    ...
    -- filter q
    params:add_control(i.."filter_q", i.." filter q", controlspec.new(0.1, 5.0, 'exp', 0.01, 2.0, ""))
...
phase = function(n, x)
  ...
  if x ~= track[n].pos_grid then
    track[n].pos_grid = x
    if view == vCUT then dirtygrid=true end
    if view == vREC then dirtygrid=true end --(added to keep grid refreshing on REC page --> CUT focused track on bottom row)
  end
end
...
gridkey_nav = function(x,z)
  if z==1 then
    ...
    elseif x==14 and alt == 1 then
      retrig() --retrig all playing tracks to pos 1
    elseif x==13 and alt == 1 then altrun() --stops all running tracks and runs all stopped tracks
    end
  elseif z==0 then
    if x==16 then alt = 0
    elseif x==14 and alt == 0 then alt2 = 0
...
gridredraw_nav = function()
  -- indicate view
  g:led(1,1,3) g:led(2,1,3) g:led(3,1,3)
  g:led(view,1,12)
  g:led(14,1,2)
  if alt==1 then g:led(16,1,15)
  ...
  --retrig
  if x == 14 and alt == 1 then g:led(14,1,15) end
end
...
v.redraw[vREC] = function()
  ...
  elseif pageNum == 3 then
    ...
    --screen.move(70,32)
    --screen.text(params:string(focus.."param"))
    screen.level(3)
    screen.move(10,40)
  ...
  elseif y>1 and y<8 then
    if z == 1 then
      ...
      elseif x==16 and y<TRACKS+2 and alt == 1 then
        print("mute track "..i) -- add function
  ...
  elseif y == 8 then return
    ...
    if z == 1 then
      ...
      if alt == 1 and y<TRACKS+2 then
        ...
        event(e)
      elseif alt2 == 1 and y<TRACKS+2 then --freeze track to one step
          first[y] = x
          e = {}
          e.t = eLOOP
          e.i = i
...
v.enc[vLFO] = function(n,d)
  if viewinfo[vLFO] == 0 then
    ...
    elseif n==3 then
      params:delta(i.."lfo_shape",d)
    end
  else
    if n==2 then
      params:delta(i.."lfo_freq",d)
    elseif n==3 then
      params:delta(i.."offset",d)
    end
  end
end
...
v.redraw[vLFO] = function()
  ...
  screen.level(sel and 15 or 4)
  screen.move(10,32)
  screen.text(params:string(i.."lfo_target"))
  screen.move(70,32)
  screen.text(params:string(i.."lfo_shape"))
  screen.level(3)
  screen.move(10,40)
  screen.text("target")
  screen.move(70,40)
  ...
  screen.level(not sel and 15 or 4)
  screen.move(10,52)
  screen.text(params:string(i.."lfo_freq"))
  screen.move(70,52)
  screen.text(params:string(i.."offset"))
  screen.level(3)
  screen.move(10,60)
  screen.text("freq")
  screen.move(70,60)
